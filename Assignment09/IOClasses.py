...
#JHsu, 2020-Mar-19, Started to work on TODO lists 
#------------------------------------------#
        ...
        try:
            with open(file_name_CD, 'r') as file:
                for line in file:
                    ...
                    new_CD = DC.CD(data[0], data[1], data[2])
...
class ScreenIO:
    ...
    def get_track_info():
        ...
        trackId = input('Enter Position on CD / Album: ').strip()
        trackTitle = input('What is the Track\'s title? ').strip()
        trackLength = input('What is the Track\'s length? ').strip()
        return trackId, trackTitle, trackLength
